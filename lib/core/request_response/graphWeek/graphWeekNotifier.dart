...
class GraphWeekNotifier extends BaseChangeNotifier {
  // final SharedPref _prefs = locator.get<SharedPref>();
  bool isloading = false;
  List<WeeklyReportData> graphWeeklyClass = [];
  List<WeeklyReportData> get GraphWeeklyClass => graphWeeklyClass;

  set GraphWeeklyClass(List<WeeklyReportData> value) {
    graphWeeklyClass = value;
    notifyListeners();
  }

  Future<void> graphWeekly(int id) async {
    final prefs = await SharedPreferences.getInstance();
    isLoading = true;
    String url = DataConstants.LIVE_BASE_URL + DataConstants.GraphWeekly;
    print(url);
    try {
      ...
      if (response.statusCode == 200) {
        final voucherTypeModel = graphWeekResponseFromJson(response.body);
        GraphWeeklyClass = voucherTypeModel.weeklyReport!;
