...
class UpdateDrawerStaffProfileNotifier extends BaseChangeNotifier {
  ...
  Future<void> updateDrawerStaffProfileInfo(
    int id,
    String fullName,
    String email,
    ...
    if (responsed.statusCode == 200) {
      try {
        final updateDrawerStaffResponse =
            updateStaffProfileResponseFromJson(responsed.body);
        if (updateDrawerStaffResponse.status == true) {
          MySharedPreferences.instance.setDoctorName(
              'doctorName', updateDrawerStaffResponse.data!.name.toString());
          MySharedPreferences.instance.setDoctorPhone(
              'doctorPhone', updateDrawerStaffResponse.data!.phone.toString());
          MySharedPreferences.instance.setAddress('doctorAddress',
              updateDrawerStaffResponse.data!.address.toString());
          MySharedPreferences.instance.setImage(
              'doctorImage', updateDrawerStaffResponse.data!.image.toString());
          appShowToast(updateDrawerStaffResponse.message!);
        } else
          appShowToast(updateDrawerStaffResponse.message!);
      } catch (e) {
        print(e);
