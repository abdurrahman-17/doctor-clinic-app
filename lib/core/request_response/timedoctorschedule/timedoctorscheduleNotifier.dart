...
class TimeDoctorScheduleNotifier extends BaseChangeNotifier {
  TimeDoctorScheduleResponse? timeDoctorScheduleResponse;
  ...
  Future<void> timeDoctorSchedule(
      int doctorId,
      String scheduleName,
      int slotTime,
      int slotType,
    ...
    String url = DataConstants.LIVE_BASE_URL + DataConstants.TimeDoctorScheduler;
    print(url);
    final input = {
      'doctor_id': doctorId,
    ...
    if (response != null && response.statusCode == 200) {
      print("saadh end");
      try {
        final timeDoctorScheduleResponse =
        timeDoctorScheduleResponseFromJson(response.body);
        if (timeDoctorScheduleResponse.status == true) {
          Get.toNamed(RoutePaths.ListAvailability, arguments: true);
        } else {
          appShowToast(timeDoctorScheduleResponse.message!);
        }
      } catch (e) {
        print(e);
