import 'package:doctor_clinic_token_app/View/DoneAppointmentData/doneAppointmentData.dart';
...
class DoneAppointment1 extends StatefulWidget {
  const DoneAppointment1({Key? key}) : super(key: key);

  @override
  _DoneAppointment1State createState() => _DoneAppointment1State();
}

class _DoneAppointment1State extends State<DoneAppointment1> {
  TodayDoneAppointmentNotifier provider = TodayDoneAppointmentNotifier();
  List<DoneAppointmentData> sortedSession = [];
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () {
      provider.todayDoneAppointment(doctorId).then((value) {
        provider.notifyListeners();
        for (int i = 0; i < provider.todayDoneAppointmentClass.length; i++) {
          print(provider.todayDoneAppointmentClass[i].prescription!);
          print('Pugvenhjcdsn');
          imageList = provider.todayDoneAppointmentClass[i].prescription!;
          print(imageList);
          print('Noiiiiis');
        }
        for (int i = 0; i < provider.todayDoneAppointmentClass.length; i++) {
          if (provider.todayDoneAppointmentClass[i].session == 'Morning') {
            sortedSession.add(provider.todayDoneAppointmentClass[i]);
          }
        }
        for (int i = 0; i < provider.todayDoneAppointmentClass.length; i++) {
          if (provider.todayDoneAppointmentClass[i].session == 'Afternoon') {
            sortedSession.add(provider.todayDoneAppointmentClass[i]);
          }
        }
        for (int i = 0; i < provider.todayDoneAppointmentClass.length; i++) {
          if (provider.todayDoneAppointmentClass[i].session == 'Evening') {
            sortedSession.add(provider.todayDoneAppointmentClass[i]);
          }
        }
        for (int i = 0; i < provider.todayDoneAppointmentClass.length; i++) {
          if (provider.todayDoneAppointmentClass[i].session == 'Night') {
            sortedSession.add(provider.todayDoneAppointmentClass[i]);
          }
        }
      });
                      ...
                      if (value == true) {
                        Future.delayed(Duration.zero, () {
                          provider.todayDoneAppointment(doctorId).then((value) {
                            provider.notifyListeners();
                            for (int i = 0;
                                i < provider.todayDoneAppointmentClass.length;
                                i++) {
                              print(provider
                                  .todayDoneAppointmentClass[i].prescription!);
                              print('Pugvenhjcdsn');
                              imageList = provider
                                  .todayDoneAppointmentClass[i].prescription!;
                              print(imageList);
  ...
  Widget build(BuildContext context) {
    if (!kReleaseMode) {}
    return Consumer<TodayDoneAppointmentNotifier>(
  ...
  Widget buildBody(context) {
    return listViewDoneAppointment(context);
  }

  Widget listViewDoneAppointment(context) {
                    ...
                    Navigator.push(
                      ...
                      PageRouteBuilder(
                        pageBuilder: (context, animation, secondaryAnimation) =>
                          ...
                          id: sortedSession[index].id,
                          pulseRate: sortedSession[index].pulseRate ?? "",
                          spo2: sortedSession[index].spo2 ?? "",
                          temperature: sortedSession[index].temperature ?? "",
                          doctorFeedback: sortedSession[index].feedback ?? "",
                          prescription: sortedSession[index].prescription ?? [],
                                        ...
                                        Expanded(
                                          ...
                                          child: Text(
                                            sortedSession[index]
                                                ...
                                                .toString(),
                                            // provider1
                                            //     .todayDoneAppointmentClass[index].patientName
                                            //     .toString(),
                                            style: const TextStyle(
